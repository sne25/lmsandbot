...
if ($_POST) {
    ...
    if ($number == $total) {
        header("Location: final.php?n=" . $next . "&user_id=$userid&semester=$semester&subject_id=$subject_id&session=$session&unit=$unit");
        exit();
    } else {
        header("Location: question.php?n=" . $next . "&user_id=$userid&semester=$semester&subject_id=$subject_id&session=$session&unit=$unit");
    }
}
?>